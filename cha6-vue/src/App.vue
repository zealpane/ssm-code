...
export default {
  ...
  mounted() {
    ...
    axios.get('/api/user/member/selfInfo').then(res => {
      if (res.data.code !== 0) {
        return
      }
      // 这里要提示还有几条告警信息待处理
      const socket = io('http://ws.iot.gdatacloud.com:8868?no=' + res.data.content, {
        transports: ['websocket']
      })
      socket.on('connect', () => {
        console.log('connect')
      })
      socket.on('commandRsp', (data) => {
        console.log(data)
        this.$notify({
          title: '指令发送状态变化',
          message: this.cmdStatus[data.result.resultCode],
          type: 'info'
        })
      })
      socket.on('workOrderEvent', () => {
        this.$notify({
          title: '有新的工单任务',
          // message: this.cmdStatus[data.result.resultCode],
          type: 'info'
      ...
      socket.on('alarmEvent', (data) => {
        ...
        this.$notify({
          title: '有新的告警信息',
          dangerouslyUseHTMLString: true,
          message: this.alarmType[data.alarmType],
          type: 'warning',
          // duration: 0,
          onClose: () => {
            this.myAuto.pause()
      ...
      // 巡检推送
      socket.on('inspectEvent', (data) => {
        console.log('接收到巡检任务', data)
